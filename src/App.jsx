...
// ─── STORAGE ───────────────────────────────────────────────────────
const KEYS = { cases:"avukat_cases", expenses:"avukat_expenses", incomes:"avukat_incomes", lawyers:"avukat_lawyers" };
...
export default function AvukatApp() {
  ...
  useEffect(()=>{(async()=>{
    const [c,e,i,l]=await Promise.all([load(KEYS.cases),load(KEYS.expenses),load(KEYS.incomes),load(KEYS.lawyers)]);
    if(c)setCases(c);if(e)setExpenses(e);if(i)setIncomes(i);if(l)setLawyers(l);
    setReady(true);
  })();},[]);
  useEffect(()=>{if(ready)save(KEYS.cases,cases);},[cases,ready]);
  useEffect(()=>{if(ready)save(KEYS.expenses,expenses);},[expenses,ready]);
  useEffect(()=>{if(ready)save(KEYS.incomes,incomes);},[incomes,ready]);
  useEffect(()=>{if(ready)save(KEYS.lawyers,lawyers);},[lawyers,ready]);
  ...
  const addAutoTax=(r)=>{
    const toAdd=[];
    if(r.kdv>0) toAdd.push({id:Date.now()+1,category:"Vergi & Stopaj",subCategory:"KDV Ödemesi",amount:r.kdv,date:r.date,note:`SMM ${r.receiptNo} KDV`,caseId:r.caseId,autoGenerated:true});
    if(r.stopaj>0) toAdd.push({id:Date.now()+2,category:"Vergi & Stopaj",subCategory:"Stopaj Kesintisi",amount:r.stopaj,date:r.date,note:`SMM ${r.receiptNo} Stopaj`,caseId:r.caseId,autoGenerated:true});
    if(toAdd.length) setExpenses(p=>[...p,...toAdd]);
  };
  ...
  const filteredCases=cases.filter(c=>{
    const txt=!search||[c.title,c.plaintiff,c.defendant,c.client,c.internalNo].some(v=>v?.toLowerCase().includes(search.toLowerCase()));
    const stg=fStage==="Tümü"||c.stage===fStage;
    const cli=fClient==="Tümü"||c.client===fClient;
    let lw=true;
    if(fLawyer!=="Tümü"){
      const id=fLawyer;
      if(fRole==="owner") lw=String(c.ownerLawyerId)===id;
      else if(fRole==="handler") lw=String(c.handlerLawyerId)===id;
      else lw=String(c.ownerLawyerId)===id||String(c.handlerLawyerId)===id;
    }
    ...
    <div style={{...S.app,flexDirection:"column"}}>
        ...
        <div style={S.content}>
          ...
          {tab==="cases"&&(
            <div style={{display:"flex",flexDirection:"column",gap:"0.9rem"}}>
              ...
              {filteredCases.map(c=>{
                const cInc=incomes.filter(i=>String(i.caseId)===String(c.id)).reduce((s,i)=>s+(+i.amount||0),0);
                const cExp=expenses.filter(e=>String(e.caseId)===String(c.id)).reduce((s,e)=>s+(+e.amount||0),0);
                const ow=getLawyer(c.ownerLawyerId),hd=getLawyer(c.handlerLawyerId);
                return (
                    ...
                    <div key={l.id} style={{background:"#0d1420",border:`1px solid ${l.color}33`,borderRadius:14,padding:"1.25rem",position:"relative"}}>
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:"0.9rem"}}>
                        ...
                        <div style={{display:"flex",gap:5}}>
                          ...
                          <button onClick={()=>{if(confirm(`${l.name} silinsin mi?`)){setLawyers(p=>p.filter(x=>x.id!==l.id));showToast("Avukat silindi.");}}} style={{background:"#1e2d45",border:"none",color:"#f87171",borderRadius:7,width:28,height:28,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Icon name="trash" size={12}/></button>
                        </div>
                      </div>
      ...
      {(modal?.type==="addCase"||modal?.type==="editCase")&&(
        <CaseForm initial={modal.data} lawyers={lawyers}
          onSave={c=>{if(modal.type==="editCase")setCases(p=>p.map(x=>x.id===c.id?c:x));else setCases(p=>[...p,c]);setModal(null);showToast(modal.type==="editCase"?"Dava güncellendi.":"Dava eklendi.");}}
          onClose={()=>setModal(null)}/>
      )}
      {(modal?.type==="addLawyer"||modal?.type==="editLawyer")&&(
        <LawyerForm initial={modal.data} usedColors={lawyers.map(l=>l.color)}
          onSave={l=>{if(modal.type==="editLawyer")setLawyers(p=>p.map(x=>x.id===l.id?l:x));else setLawyers(p=>[...p,l]);setModal(null);showToast("Avukat kaydedildi.");}}
          onClose={()=>setModal(null)}/>
      )}
      {modal?.type==="smm"&&<ReceiptForm cases={cases} onSave={r=>{setIncomes(p=>[...p,r]);addAutoTax(r);setModal(null);showToast("Makbuz kaydedildi + vergi kalemleri otomatik eklendi.");}} onClose={()=>setModal(null)}/>}
      {modal?.type==="otherIncome"&&<OtherIncomeForm cases={cases} onSave={i=>{setIncomes(p=>[...p,i]);setModal(null);showToast("Gelir eklendi.");}} onClose={()=>setModal(null)}/>}
      {modal?.type==="viewReceipt"&&<ReceiptView r={modal.data} firm={firmInfo} onClose={()=>setModal(null)}/>}
      {modal?.type==="addExpense"&&<ExpenseForm cases={cases} onSave={e=>{setExpenses(p=>[...p,e]);setModal(null);showToast("Gider eklendi.");}} onClose={()=>setModal(null)}/>}
