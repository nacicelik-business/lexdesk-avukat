...
import { fetchAll, insertRow, updateRow, deleteRow, supabase, uploadDocument, getDocumentUrl, deleteDocument, fetchDocuments, extractTextFromFile, fetchPaymentSchedule, fetchAllPaymentSchedules, getOrCreateWorkspace, updateWorkspace } from "./supabase.js";
...
function LoginScreen({ onLogin }) {
  ...
  const handle = async () => {
    ...
    try {
      if (mode === "register") {
        const wsName = firmName.trim() || name.trim() + " Hukuk Bürosu";
        const { error: e } = await supabase.auth.signUp({
          email, password,
          options: { data: { full_name: name, firm_name: wsName } }
        });
        if (e) throw e;
      ...
      <div style={S.box}>
        ...
        {mode==="register" && (
          <>
            ...
            <label style={S.lbl}>Hukuk Bürosu Adı</label>
            <input style={S.inp} value={firmName} onChange={e=>setFirmName(e.target.value)} placeholder="Yılmaz Hukuk Bürosu"/>
          </>
        )}
...
function MainPanel({ session, profile }) {
  const workspace = { id: profile?.workspace_id || 1, name: "Hukuk Bürosu" };
  const [workspaceName, setWorkspaceName] = useState("Hukuk Bürosu");

  useEffect(()=>{
    if(profile?.workspace_id) {
      supabase.from("workspaces").select("name").eq("id", profile.workspace_id).single()
        .then(({data})=>{ if(data?.name) setWorkspaceName(data.name); });
    }
  },[profile?.workspace_id]);

  const ws = { ...workspace, name: workspaceName };
  // ── STATE (önce tanımla) ───────────────────────────────────────
    ...
    <div style={{...S.app,flexDirection:"column"}}>
      ...
      <div style={{...S.navbar,flexDirection:"row"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,marginRight:8}}>
          {ws?.logo_url
            ? <img src={ws.logo_url} alt="logo" style={{height:28,maxWidth:100,objectFit:"contain"}}/>
            : <div style={{width:28,height:28,background:"linear-gradient(135deg,#b8962e,#e2c97e)",borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon name="scale" size={14}/></div>
          }
          <div style={{display:"flex",flexDirection:"column",lineHeight:1.25}}>
            {(()=>{
              const wn = ws?.name || workspaceName || "LexDesk";
              const hukukIdx = wn.toLowerCase().indexOf("hukuk");
              if(hukukIdx > 0) {
                return <>
                  <span style={{color:"#e2c97e",fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:15,whiteSpace:"nowrap"}}>{wn.slice(0,hukukIdx).trim()}</span>
                  <span style={{color:"#9ca3af",fontSize:12,whiteSpace:"nowrap",fontWeight:500}}>{wn.slice(hukukIdx).trim()}</span>
                </>;
              }
              return <span style={{color:"#e2c97e",fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:15,whiteSpace:"nowrap"}}>{wn}</span>;
            })()}
          </div>
        </div>
      ...
      {(modal?.type==="addCase"||modal?.type==="editCase")&&(
        <CaseForm initial={modal.data} lawyers={lawyers}
          onSave={async c=>{
            const payload={workspace_id:workspace?.id,title:c.title,type:c.type,stage:c.stage,plaintiff:c.plaintiff,defendant:c.defendant,court:c.court,file_no:c.fileNo,internal_no:c.internalNo,case_value:+c.caseValue||0,side:c.side,importance:+c.importance||5,win_rate:+c.winRate||50,risk_amount:+c.riskAmount||0,client:c.client,work_hours:+c.workHours||0,open_date:c.openDate||null,next_date:c.nextDate||null,expected_fee:+c.expectedFee||0,notes:c.notes,photo:c.photo,owner_lawyer_id:c.ownerLawyerId?+c.ownerLawyerId:null,handler_lawyer_id:c.handlerLawyerId?+c.handlerLawyerId:null};
      ...
      {(modal?.type==="addLawyer"||modal?.type==="editLawyer")&&(
        <LawyerForm initial={modal.data} usedColors={lawyers.map(l=>l.color)}
          onSave={async l=>{
            const payload={workspace_id:workspace?.id,name:l.name,title:l.title,bar_no:l.barNo,phone:l.phone,email:l.email,color:l.color,photo:l.photo,notes:l.notes};
      ...
      {modal?.type==="otherIncome"&&<OtherIncomeForm cases={cases} onSave={async i=>{
        const payload={workspace_id:workspace?.id,type:i.type,date:i.date,client_name:i.clientName,amount:+i.amount,note:i.note,case_id:i.caseId?+i.caseId:null};
        const row=await insertRow("incomes",payload);
      ...
      {modal?.type==="addExpense"&&<ExpenseForm cases={cases} onSave={async e=>{
        const payload={workspace_id:workspace?.id,category:e.category,sub_category:e.subCategory,amount:+e.amount,date:e.date,note:e.note,case_id:e.caseId?+e.caseId:null};
...
function PaymentScheduleModal({ caseData, onClose }) {
  ...
  const markPaid = async (payment) => {
    const paidAmount = prompt("Tahsil edilen tutar:", payment.amount)
  ...
  const markUnpaid = async (payment) => {
    const updated = await updateRow("payment_schedule", payment.id, { paid: false, paid_date: null, paid_amount: 0 })
    if (updated) setPayments(p => p.map(x => x.id===payment.id ? updated : x))
  }
  ...
  const extractFromDocs = async () => {
    ...
    try {
      ...
      if (!summaries) { alert("Önce belgeler klasörüne belge yükleyin."); setExtracting(false); return }
      const res = await fetch("/api/claude", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          model:"claude-sonnet-4-5", max_tokens:1000,
          system:"Sen bir hukuk asistanısın. Belge özetlerinden ödeme tarihlerini çıkar. Sadece JSON döndür.",
          messages:[{ role:"user", content:`Ödeme takvimini çıkar. Sadece JSON:\n{"payments":[{"description":"açıklama","amount":12000,"due_date":"2024-03-15"}]}\n\nBelgeler:\n${summaries}` }]
        })
      })
      const data = await res.json()
      ...
      if (parsed.payments?.length > 0) {
        for (const p of parsed.payments) {
          ...
          const row = await insertRow("payment_schedule", { case_id:caseData.id, description:p.description||"Ödeme", amount:+p.amount, due_date:p.due_date })
      ...
      } else { alert("Belgelerden ödeme tarihi bulunamadı.") }
  ...
  const exportToCalendar = (payment) => {
    ...
    const ics = ["BEGIN:VCALENDAR","VERSION:2.0","BEGIN:VEVENT",
      ...
      `DESCRIPTION:Beklenen: ${fmt(payment.amount)}`,
      "BEGIN:VALARM","TRIGGER:-P1D","ACTION:DISPLAY","DESCRIPTION:Ödeme Hatırlatma","END:VALARM",
      "END:VEVENT","END:VCALENDAR"].join("\r\n")
    const blob = new Blob([ics], { type:"text/calendar" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a"); a.href=url; a.download=`odeme_${payment.due_date}.ics`; a.click()
  ...
  const fmt2 = n => new Intl.NumberFormat("tr-TR",{style:"currency",currency:"TRY",maximumFractionDigits:0}).format(n||0)
  ...
  return (
    <Modal title={`💰 Ödeme Takvimi — ${caseData.title}`} onClose={onClose} wide>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"0.65rem",marginBottom:"1.25rem"}}>
        {[{l:"Toplam Beklenen",v:fmt2(totalExpected),c:"#e2c97e"},{l:"Tahsil Edilen",v:fmt2(totalPaid),c:"#10b981"},{l:"Kalan Alacak",v:fmt2(totalPending),c:totalPending>0?"#f87171":"#10b981"}].map(s=>(
          <div key={s.l} style={{background:"#0a1628",border:`1px solid ${s.c}33`,borderRadius:10,padding:"0.75rem",textAlign:"center"}}>
      ...
      {overdue.length > 0 && (
        <div style={{background:"#7f1d1d22",border:"1px solid #991b1b",borderRadius:8,padding:"0.65rem 1rem",marginBottom:"1rem",fontSize:13,color:"#fca5a5"}}>
          ⚠️ {overdue.length} adet vadesi geçmiş ödeme — {fmt2(overdue.reduce((s,p)=>s+(+p.amount||0),0))}
        </div>
      )}

      {loading ? <div style={{textAlign:"center",color:"#6b7280",padding:"2rem"}}>⏳</div>
      : payments.length === 0 ? <div style={{textAlign:"center",color:"#4b5563",padding:"2rem",fontSize:13}}>Henüz ödeme takvimi yok.</div>
      : <div style={{display:"flex",flexDirection:"column",gap:"0.5rem",marginBottom:"1rem"}}>
          {payments.map(p => (
            <div key={p.id} style={{display:"flex",alignItems:"center",gap:"0.75rem",background:p.paid?"#064e3b22":"#0d1420",border:`1px solid ${statusColor(p)}44`,borderLeft:`3px solid ${statusColor(p)}`,borderRadius:"0 10px 10px 0",padding:"0.75rem 1rem",opacity:p.paid?0.75:1}}>
              <div style={{fontSize:18,flexShrink:0}}>{p.paid?"✅":new Date(p.due_date)<new Date()?"🔴":"💳"}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:600,color:p.paid?"#9ca3af":"#e5e7eb",textDecoration:p.paid?"line-through":"none"}}>{p.description}</div>
              ...
              <div style={{fontSize:14,fontWeight:800,color:p.paid?"#10b981":statusColor(p),flexShrink:0}}>{fmt2(p.paid&&p.paid_amount?p.paid_amount:p.amount)}</div>
              <div style={{display:"flex",gap:4,flexShrink:0}}>
                {!p.paid && <button onClick={()=>markPaid(p)} style={{background:"#10b98122",border:"1px solid #10b98144",color:"#10b981",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11}}>✓ Ödendi</button>}
                {p.paid && <button onClick={()=>markUnpaid(p)} style={{background:"#1e2d45",border:"none",color:"#6b7280",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11}}>Geri Al</button>}
                <button onClick={()=>exportToCalendar(p)} title="Takvime Ekle" style={{background:"#1e2d45",border:"none",color:"#a78bfa",borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:13}}>📅</button>
                <button onClick={async()=>{if(confirm("Sil?")){await deleteRow("payment_schedule",p.id);setPayments(prev=>prev.filter(x=>x.id!==p.id))}}} style={{background:"none",border:"none",color:"#4b5563",cursor:"pointer"}}><Icon name="trash" size={12}/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {addMode && (
        <div style={{background:"#0a1628",border:"1px solid #1e3a5f",borderRadius:10,padding:"1rem",marginBottom:"1rem"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 130px 140px",gap:"0.65rem",alignItems:"end"}}>
            <Input label="Açıklama" value={newPayment.description} onChange={e=>setNewPayment(p=>({...p,description:e.target.value}))} placeholder="1. Taksit"/>
            <Input label="Tutar (₺)" type="number" value={newPayment.amount} onChange={e=>setNewPayment(p=>({...p,amount:e.target.value}))}/>
            <Input label="Vade" type="date" value={newPayment.due_date} onChange={e=>setNewPayment(p=>({...p,due_date:e.target.value}))}/>
          </div>
      ...
      <div style={{display:"flex",gap:"0.65rem",flexWrap:"wrap",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",gap:"0.65rem"}}>
          ...
          <Btn small variant="ai" onClick={extractFromDocs}>{extracting?"⏳ Çıkarılıyor...":"🤖 Belgeden Çıkar"}</Btn>
        </div>
