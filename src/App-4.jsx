...
export default function AvukatApp() {
  ...
  useEffect(()=>{
    if(!session?.user) { setProfile(null); setWorkspace(null); return; }
    supabase
      .from("profiles")
      .select("*, workspaces(*)")
      ...
      .then(({data, error})=>{
        if(data) {
          setProfile(data);
          setWorkspace(data.workspaces || { id: data.workspace_id, name: "Hukuk Bürosu" });
        }
      });
  },[session]);
  ...
  return <MainPanel session={session} profile={profile} workspace={workspace} setWorkspace={setWorkspace} />;
}

// ─── ANA PANEL (auth sonrası) ──────────────────────────────────────
function MainPanel({ session, profile, workspace, setWorkspace }) {
  // ── STATE (önce tanımla) ───────────────────────────────────────
    ...
    <div style={{...S.app,flexDirection:"column"}}>
      ...
      <div style={{...S.navbar,flexDirection:"row"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,marginRight:8}}>
          {workspace?.logo_url
            ? <img src={workspace.logo_url} alt="logo" style={{height:28,maxWidth:100,objectFit:"contain"}}/>
          ...
          <span style={{color:"#e2c97e",fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:14,whiteSpace:"nowrap"}}>{workspace?.name||"LexDesk"}</span>
        </div>
